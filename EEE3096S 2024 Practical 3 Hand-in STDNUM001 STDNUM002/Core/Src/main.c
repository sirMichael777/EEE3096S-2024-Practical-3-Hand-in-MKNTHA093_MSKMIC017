...
int main(void)
{
  ...
  for (uint16_t i = 0; i < 6; i++) {
      write_to_address(i, binaryValues[i]);
      delay(300);
  }
  ...
  while (1)
  {
	...
	// TODO: Poll ADC
	uint32_t pollADCVal = pollADC();

	// TODO: Get CRR
	CCR = ADCtoCCR(pollADCVal);
...
void TIM16_IRQHandler(void)
{
    ...
    // Read the next binary value from EEPROM using SPI
    uint8_t read_value = read_from_address(eeprom_address);

    // Check if the read value matches the expected value
    char lcd_output[16]; // Buffer for LCD output
    if (read_value == binaryValues[eeprom_address]) {
        // If the value is correct, display it on the LCD
        sprintf(lcd_output, "%d", read_value);  // Convert to decimal and store in buffer
        writeLCD(lcd_output);  // Write to LCD
    } else {
        // If there is a mismatch, display SPI error on LCD
        sprintf(lcd_output, "SPI ERROR! %d", read_value);  // Show the erroneous value
        writeLCD(lcd_output);  // Write error to LCD
    }

    // Increment EEPROM address and index for the next read
    eeprom_address++;

    // If the address/index exceeds the number of EEPROM values, reset to 0
    if (eeprom_address >= 6) {
        eeprom_address = 0;
    }
...
void writeLCD(char *char_in){
  ...
  /*float number = 123456.3;
  char buffer[16]; // Ensure this buffer is large enough to hold the string
